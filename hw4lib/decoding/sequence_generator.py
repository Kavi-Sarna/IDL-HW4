...
class SequenceGenerator:
    ...
    def generate_beam(
        ...
        # raise NotImplementedError # Remove once implemented
        # Special case: beam_width=1 is equivalent to greedy search
        if beam_width == 1:
            sequences, scores = self.generate_greedy(x, temperature, repeat_penalty)
            return sequences.unsqueeze(1), scores.unsqueeze(1)
    
        batch_size = x.size(0)
        vocab_size = self.score_fn(x[:1]).size(-1)  # Get vocabulary size
    
        # Initialize beam state
        # For each batch item, we'll maintain beam_width sequences
        current_sequences = x.repeat_interleave(beam_width, dim=0)  # (batch_size * beam_width, seq_len)
        beam_scores = torch.zeros(batch_size, beam_width, device=x.device)  # (batch_size, beam_width)
        beam_finished = torch.zeros(batch_size, beam_width, dtype=torch.bool, device=x.device)  # (batch_size, beam_width)
        ...
        for step in range(self.max_length - x.size(1)):
            ...
            # Get logits for next tokens
            next_logits = self.score_fn(current_sequences)  # (batch_size * beam_width, vocab_size)
        
            # Reshape for batch processing
            next_logits = next_logits.view(batch_size, beam_width, vocab_size)
        
            # Apply repeat penalty if needed
            if repeat_penalty != 1.0:
                current_sequences_view = current_sequences.view(batch_size, beam_width, -1)
                next_logits = self._apply_repeat_penalty(next_logits, current_sequences_view, repeat_penalty)
            ...
            # Apply softmax to get log probabilities
            log_probs = torch.log_softmax(next_logits, dim=-1)  # (batch_size, beam_width, vocab_size)
        
            # Calculate scores for all possible next tokens
            # For each batch and beam, we need to compute: current_score + next_token_score
            vocab_scores = beam_scores.unsqueeze(-1) + log_probs  # (batch_size, beam_width, vocab_size)
        
            # For finished beams, only the EOS token should be considered
            for batch_idx in range(batch_size):
                for beam_idx in range(beam_width):
                    if beam_finished[batch_idx, beam_idx]:
                        # Set all scores to -inf except for EOS
                        vocab_scores[batch_idx, beam_idx, :] = float('-inf')
                        vocab_scores[batch_idx, beam_idx, self.tokenizer.eos_id] = beam_scores[batch_idx, beam_idx]
        
            # Flatten vocab_scores for top-k selection
            vocab_scores_flat = vocab_scores.view(batch_size, -1)  # (batch_size, beam_width * vocab_size)
        
            # Select top-k for each batch
            top_scores, top_indices = torch.topk(vocab_scores_flat, beam_width, dim=-1)  # (batch_size, beam_width)
        
            # Convert flat indices to beam indices and token indices
            beam_indices = top_indices // vocab_size  # (batch_size, beam_width)
            token_indices = top_indices % vocab_size  # (batch_size, beam_width)
            ...
            for batch_idx in range(batch_size):
                for beam_idx in range(beam_width):
                    ...
                    # Get the token to append
                    next_token = token_indices[batch_idx, beam_idx]
                    # Get the parent sequence
                    parent_seq = current_sequences[batch_idx * beam_width + parent_beam]
                    # Create new sequence by appending the next token
                    new_seq = torch.cat([parent_seq, next_token.unsqueeze(0)], dim=0)
                    new_sequences.append(new_seq)
        
            # Stack the new sequences
            current_sequences = torch.stack(new_sequences).view(batch_size * beam_width, -1)
        
            # Update beam_finished
            for batch_idx in range(batch_size):
                for beam_idx in range(beam_width):
                    if token_indices[batch_idx, beam_idx] == self.tokenizer.eos_id:
                        beam_finished[batch_idx, beam_idx] = True
    
        # Reshape sequences to (batch_size, beam_width, sequence_length)
        final_sequences = current_sequences.view(batch_size, beam_width, -1)
    
        return final_sequences, beam_scores
